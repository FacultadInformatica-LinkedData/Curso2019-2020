...
public class Task07
{
	...
	public static void main(String args[])
	{
		...
		//Lista instancias
		ExtendedIterator instancias = modelInf.getOntClass(ns + "Person").listInstances();
		
		//Lista subclases
		ExtendedIterator subClases = modelInf.getOntClass(ns + "Person").listSubClasses();
		
		//Mientras tenga siguiente
		while (instancias.hasNext()){
			Individual instancia = (Individual) instancias.next();
			System.out.println(instancia.getURI());
		}
		
		while (subClases.hasNext()){
			OntClass subClase1 = (OntClass) subClases.next();
			System.out.println(subClasel.getURI());
		}
